...
const Post = ({ post }) => {
    ...
    const [postLike, setPostLike] = useState(post.reactions.length);
    ...
    const likeOrDislikeHandler = async () => {
        try {
            ...
            const res = await axios.get(`${BACKEND_URL}/api/v1/post/${post._id}/${action}`, { withCredentials: true });
    ...
    const bookmarkHandler = async () => {
        try {
            const res = await axios.get(`${BACKEND_URL}/api/v1/post/${post?._id}/bookmark`, {withCredentials:true});
            if(res.data.success){
                toast.success(res.data.message);
            }
        } catch (error) {
